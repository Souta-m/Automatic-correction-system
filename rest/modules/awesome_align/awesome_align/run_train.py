...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    def backward_loss(loss, tot_loss):  #lossを足していく機能
        if args.n_gpu > 1:
    ...
    for _ in range(int(args.num_train_epochs)):
        for step, batch in enumerate(train_dataloader):
            ...
            if args.train_so or args.train_co:
                ...
                tr_loss = backward_loss(loss, tr_loss) #Consistency Optimizationのloss,以下ほか機能のlossである。
                print("so")
                print(loss)
            if args.train_mlm:
                inputs_src, labels_src = mask_tokens(batch[0], tokenizer, args) #ここでsrcのtokenをmask
                inputs_tgt, labels_tgt = mask_tokens(batch[1], tokenizer, args) #ここでtgtのtokenをmask
                inputs_src, inputs_tgt = inputs_src.to(args.device), inputs_tgt.to(args.device)
