...
class BertLMPredictionHead(nn.Module): #論文部 Trasnlation Language Modeling部
    def __init__(self, config):
        super().__init__()
...
class BertMLMHead(nn.Module):  #論文部 Masked Language Modeling部
    def __init__(self, config):
        super().__init__()
...
class BertPSIHead(nn.Module): #論文部 Parallel Sentence Identification部
    def __init__(self, config):
        super().__init__()
...
class BertGuideHead(nn.Module):
    ...
    def forward(
        ...
        if guide is None:
            ...
            align_matrix = (attention_probs_src>threshold)*(attention_probs_tgt>threshold) #アライメント行列作成
            if not output_prob:
                return align_matrix
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def forward(
        ...
        if inputs_tgt is None:
            ...
            return masked_lm_loss  #MLM,TLMのloss
    ...
    def get_aligned_word(self, inputs_src, inputs_tgt, bpe2word_map_src, bpe2word_map_tgt, device, src_len, tgt_len, align_layer=8, extraction='softmax', softmax_threshold=0.001, test=False, output_prob=False, word_aligns=None):
        ...
        if word_aligns is None:
            ...
            with torch.no_grad():
                ...
                attention_probs_inter = self.guide_layer(outputs_src, outputs_tgt, inputs_src, inputs_tgt, extraction=extraction, softmax_threshold=softmax_threshold, output_prob=output_prob) #attention確率の計算
                if output_prob:
            ...
            for idx, (attention, b2w_src, b2w_tgt) in enumerate(zip(attention_probs_inter, bpe2word_map_src, bpe2word_map_tgt)):
                ...
                # all_prob=[]
                # tmp_prob=[]
                # for i in range(attention_probs_inter.size(1)):
                #     for j in range(attention_probs_inter.size(2)):
                #         word_pair = (b2w_src[i], b2w_tgt[j])
                #         if output_prob:
                #             prob = alignment_probs[idx, i, j]
                #             if not word_pair in aligns:
                #                 tmp_prob.append(prob)
                #                 prob=max(tmp_prob)
                #                 all_prob.append(prob.item())
                #                 tmp_prob=[]
                #             else:
                #                 tmp_prob.append(prob)
                #         else:
